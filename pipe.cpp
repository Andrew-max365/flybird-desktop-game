...
Pipe::Pipe() : speed(4.0), gap(180),isPassed(false),MaxSpeed(10.0) {
	// 随机生成管道高度
...
void Pipe::movePipe(double totalTime) {
	// 管道向左移动
    double newspeed = std::min(speed + 0.2 * totalTime , MaxSpeed);   //每过1s增加0.2单位速度,但最大速度为10
    setX(x() - newspeed);
}
